...
This blog is built using a static site generator. How does it work?

This question is best answered by tracing what happens when a user (you!) runs `npm run build` in the command line:

## On the server

1. After running `npm run build`, npm will look at `package.json` to find the build script which points at `./quartz/bootstrap-cli.mjs`.
...
4. In `build.ts`, we start by installing source map support manually to account for the query string cache busting hack we introduced earlier. Then, we start processing content:
   ...
   3. Parse the Markdown files.
      ...
      3. Parsing has three steps:
         ...
         3. Slugify the file path and store it in the data for the file. See the page on [[paths]] for more details about how path logic works in the site generator (spoiler: its complicated).
